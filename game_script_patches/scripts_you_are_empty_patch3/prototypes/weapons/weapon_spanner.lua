...
----------------------------------------------------------------------
function sv_weapon_spanner:on_selected()
	
	-- Переопределяем функцию для корректного отыгрывания анимаций
	self:set_update_enable(true)
	self:set_req_fixed_update_rate(false)
	self:set_update_interval(20)
	
end

----------------------------------------------------------------------
function sv_weapon_spanner:on_deselected()
...
----------------------------------------------------------------------
function cl_weapon_spanner:on_selected()

	-- Переопределяем функцию для корректного отыгрывания анимаций	
	self:set_update_enable(true)
	self:set_req_fixed_update_rate(false) --для избежания задержек во время стрельбы при низком framerate
	
end

----------------------------------------------------------------------
function cl_weapon_spanner:on_deselected()
