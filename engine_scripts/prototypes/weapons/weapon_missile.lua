...
------------------------------------------------------------------------
-- Ѕазовый класс дл€ метательного оружи€.
------------------------------------------------------------------------
...
function sv_weapon_missile:on_init()
	...
	-- дополнительное состо€ние дл€ выстрела
...
function sv_weapon_missile:can_be_selected()
	-- ≈сли нет патронов, то не позвол€ем выбирать оружие
	return not self:is_empty()
end

---------------------------------------------------------------------------
function sv_weapon_missile:event_reload()	
	-- Ќичего не делаем, в отличие от версии в sv_weapon_firearm
end
...
function sv_weapon_missile:on_update_fire()
	...
	if owner then
		...
		-- ¬рем€ удерживани€ гранаты
		local hold_time = self.m_lever_up_time - self.m_lever_down_time
		-- ќпределим скорость пропорционально времени удержани€
		local velocity
...
function sv_weapon_missile:on_update_fire_finish()
	if self:can_reload() then
		...
		-- «акончились патроны - преключаемс€ на следующее оружие (по приоритету)
		local owner = self:get_owner()
		if owner then
